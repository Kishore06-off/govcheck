...
def tokenize(text: str) -> list[str]:
    # Light, dependency-free tokenizer that behaves well on policy text.
    # Keeps alphanumerics and common separators, drops empty tokens.
    tokens = re.findall(r"[a-z0-9]+(?:[_\-/][a-z0-9]+)*", (text or "").lower())
    return [t for t in tokens if t]

def _dedupe_corpus_by_chunk_id(corpus: list[dict]) -> list[dict]:
    """Deduplicate corpus items by stable chunk_id ('id'), keeping the latest."""
    seen: dict[str, dict] = {}
    for doc in corpus:
        ...
        if not cid:
            continue
        seen[cid] = doc
    # Preserve deterministic order (insertion order of dict == last-seen order)
...
def build_bm25_index(chunks: list[dict], user_id: str = "anonymous"):
    ...
    # Append new chunks (then dedupe by chunk_id to prevent drift)
    new_corpus = [{"id": c["chunk_id"], "text": c["text"], "metadata": c} for c in chunks if c.get("chunk_id") and c.get("text")]
    user_corpus.extend(new_corpus)
    user_corpus = _dedupe_corpus_by_chunk_id(user_corpus)
